/**
 * Pagina Statistiche Esercizio Parola-Immagine
...
import {
  ...
  AlertTriangle, Loader2, Users, Filter, Printer
} from 'lucide-react'
...
function StatisticheContent() {
  ...
  const [tipoEsercizio, setTipoEsercizio] = useState<string>('')
  ...
  // Carica statistiche quando cambia utente o tipo
  useEffect(() => {
    if (selectedUserId) {
      loadStatistiche()
    }
  }, [selectedUserId, tipoEsercizio])
  ...
  const loadStatistiche = async () => {
    ...
    try {
      let url = `/api/esercizi/parola-immagine/statistiche?id_utente=${selectedUserId}`
      if (tipoEsercizio) {
        url += `&tipo_esercizio=${tipoEsercizio}`
      }
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text('Statistiche Esercizio Parola-Immagine', pageWidth / 2, yPos, { align: 'center' })
      yPos += 10
      ...
      // Tipo esercizio filtrato
      const tipoLabel = tipoEsercizio === 'parola-immagine' ? 'Parola → Immagine' :
                        tipoEsercizio === 'immagine-parola' ? 'Immagine → Parola' : 'Tutti gli esercizi'
      pdf.setFontSize(11)
      pdf.setTextColor(59, 130, 246) // blue-500
      pdf.text(`Filtro: ${tipoLabel}`, pageWidth / 2, yPos, { align: 'center' })
      yPos += 15
      ...
      const imgWidth = (pageWidth - (margin * 2) - 5) / 2 // Due grafici per riga
      const imgHeight = 55
      ...
      pdf.setFillColor(88, 28, 135) // purple-800
      pdf.rect(margin, yPos, pageWidth - (margin * 2), 7, 'F')
      pdf.text('Tipo', margin + 2, yPos + 5)
      pdf.text('#', margin + 20, yPos + 5)
      pdf.text('Data', margin + 30, yPos + 5)
      pdf.text('Prove', margin + 65, yPos + 5)
      pdf.text('OK', margin + 85, yPos + 5)
      pdf.text('Err', margin + 100, yPos + 5)
      pdf.text('%', margin + 115, yPos + 5)
      pdf.text('Tempo', margin + 135, yPos + 5)
      yPos += 7
      ...
      sessioni.forEach((s, idx) => {
        ...
        const tipo = s.tipo_esercizio === 'parola-immagine' ? 'P→I' : 'I→P'
        pdf.text(tipo, margin + 2, yPos + 4)
        pdf.text(String(s.progressivo), margin + 20, yPos + 4)
        pdf.text(s.data || '-', margin + 30, yPos + 4)
        pdf.text(String(s.totale_prove), margin + 65, yPos + 4)
        pdf.text(String(s.corrette), margin + 85, yPos + 4)
        pdf.text(String(s.errate), margin + 100, yPos + 4)
        pdf.text(`${s.percentuale}%`, margin + 115, yPos + 4)
        pdf.text(`${(s.tempo_medio_ms / 1000).toFixed(1)}s`, margin + 135, yPos + 4)
        yPos += 6
      })
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(`TrainingCognitivo - Statistiche Esercizio - Pagina ${i}/${totalPages}`, pageWidth / 2, pageHeight - 10, { align: 'center' })
      }

      // Salva PDF
      // Nome file con tipo esercizio
      const tipoNome = tipoEsercizio === 'parola-immagine' ? 'parola_immagine' :
                       tipoEsercizio === 'immagine-parola' ? 'immagine_parola' : 'tutti_esercizi'
      const nomeUtente = selectedUserName.toLowerCase().replace(/\s+/g, '_')
      const dataFile = dataStr.replace(/\//g, '_')
      const nomeFile = `${tipoNome}_${nomeUtente}_${dataFile}.pdf`
      pdf.save(nomeFile)
  ...
  const dataAndamento = sessioni.map(s => ({
    nome: `${s.tipo_esercizio === 'parola-immagine' ? 'P→I' : 'I→P'} #${s.progressivo}`,
    percentuale: s.percentuale,
    tipo: s.tipo_esercizio
  }))

  // Dati per grafico tempo di risposta
  const dataTempo = sessioni.map(s => ({
    nome: `#${s.progressivo}`,
    tempo: Math.round(s.tempo_medio_ms / 1000 * 10) / 10, // Converti in secondi
    tipo: s.tipo_esercizio
  }))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100">
      {/* Header */}
      <header className="bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg p-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {selectedUserId && sessioni.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white text-purple-700 font-bold rounded-lg hover:bg-purple-50 transition-colors shadow-md disabled:opacity-50"
              title="Stampa PDF"
            >
              {generatingPdf ? (
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="min-w-[200px]">
              ...
              <select
                value={tipoEsercizio}
                onChange={(e) => setTipoEsercizio(e.target.value)}
                className="w-full p-3 border-2 border-purple-200 rounded-xl focus:border-purple-500 focus:outline-none"
              >
                <option value="">Tutti</option>
                <option value="parola-immagine">Parola → Immagine</option>
                <option value="immagine-parola">Immagine → Parola</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl shadow-lg p-4 text-center">
                ...
                <p className="text-3xl font-bold text-purple-700">{riepilogo?.totale_sessioni}</p>
                  ...
                  <div className="space-y-2 max-h-[280px] overflow-y-auto">
                    {paroleDifficili.map((p, idx) => (
                      <div key={p.parola} className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">
                        ...
                        <span className="flex-1 font-medium">{p.parola}</span>
                      ...
                      <tr key={s.key} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="p-3">
                          <span className={`px-2 py-1 rounded text-xs font-bold ${
                            s.tipo_esercizio === 'parola-immagine'
                              ? 'bg-blue-100 text-blue-700'
                              : 'bg-purple-100 text-purple-700'
                          }`}>
                            {s.tipo_esercizio === 'parola-immagine' ? 'P→I' : 'I→P'}
                          </span>
                        </td>
