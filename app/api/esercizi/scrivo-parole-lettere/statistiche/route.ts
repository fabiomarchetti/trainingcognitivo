/**
 * API Statistiche Esercizio Scrivo Parole con le Lettere
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query risultati
    const { data: risultati, error } = await supabaseAdmin
    ...
    // Raggruppa per sessione (progressivo_esercizio)
    const sessioniMap = new Map<number, any[]>()

    risultati.forEach(r => {
      const key = r.progressivo_esercizio
    ...
    const sessioni = Array.from(sessioniMap.entries()).map(([progressivo, prove]) => {
      ...
      const tempoMedio = tempiValidi.length > 0
        ? Math.round(tempiValidi.reduce((a: number, b: number) => a + b, 0) / tempiValidi.length)
        : 0

      const prima = prove[0]

      return {
        key: `sessione_${progressivo}`,
        progressivo,
    ...
    sessioni.sort((a, b) => a.progressivo - b.progressivo)
    ...
    const riepilogo = {
      ...
      tempo_medio_ms: tempiTotali.length > 0
        ? Math.round(tempiTotali.reduce((a: number, b: number) => a + b, 0) / tempiTotali.length)
        : 0
    }
    ...
    console.error('Errore API statistiche scrivo-lettere:', error)
    return NextResponse.json({
      success: false,
